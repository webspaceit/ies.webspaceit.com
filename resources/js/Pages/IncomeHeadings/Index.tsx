...
export default function IncomeHeadings({ headings, categories }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('income-headings.store'), {
            onSuccess: () => {
                reset();
                setShowForm(false);
            },
    ...
    const handleUpdate = (e: React.FormEvent, id: number) => {
        e.preventDefault();
        editData.put(route('income-headings.update', id), {
    ...
    const handleDelete = (id: number) => {
        if (confirm('Are you sure you want to delete this heading?')) {
            router.delete(route('income-headings.destroy', id));
        }
    };

    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Income Headings</h2>}
        >
            <Head title="Income Headings" />
            ...
            <div className="py-12">
                <div className="mx-auto max-w-full px-6">
                    <Card className="mb-6">
                        <CardHeader>
                            <div className="flex items-center justify-between">
                                <CardTitle>Add Income Heading</CardTitle>
                                <button
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle>All Income Headings ({headings.length})</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        ...
                                        {headings.length === 0 && (
                                            <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-500">No income headings found.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
