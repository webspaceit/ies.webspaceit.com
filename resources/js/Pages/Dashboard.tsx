...
export default function Dashboard() {
    ...
    const currentUser = usePage().props.auth.user as any;

    const handleRoleChange = (userId: number, newRole: 'admin' | 'accountant') => {
        router.patch(route('users.role.update', userId), { role: newRole }, {
            preserveScroll: true,
        });
    };
        ...
        >
            ...
            <div className="space-y-4 sm:space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
                    {/* Income Card */}
                    <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary-500 to-primary-700 p-4 sm:p-6 text-white shadow-xl shadow-primary-500/20 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-primary-500/30">
                        ...
                        <div className="relative">
                            ...
                            <p className="text-xs sm:text-sm font-medium text-white/80">Total Income ({year})</p>
                            <p className="mt-1 text-2xl sm:text-3xl font-bold">{formatCurrency(totalIncome)}</p>
                        </div>
                    </div>

                    {/* Expense Card */}
                    <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-red-500 to-rose-600 p-4 sm:p-6 text-white shadow-xl shadow-red-500/20 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-red-500/30">
                        <div className="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-white/10"></div>
                        <div className="absolute -bottom-4 -right-4 h-16 w-16 rounded-full bg-white/5"></div>
                        <div className="relative">
                            <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-white/20 backdrop-blur-sm">
                            ...
                            <p className="text-xs sm:text-sm font-medium text-white/80">Total Expenses ({year})</p>
                            <p className="mt-1 text-2xl sm:text-3xl font-bold">{formatCurrency(totalExpenses)}</p>
                        </div>
                    </div>

                    {/* Balance Card */}
                    <div className={`group relative overflow-hidden rounded-2xl p-4 sm:p-6 text-white shadow-xl transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl ${
                        balance >= 0
                        ...
                        <div className="relative">
                            ...
                            <p className="text-xs sm:text-sm font-medium text-white/80">Net Balance ({year})</p>
                ...
                <Card className="border-0 shadow-lg">
                    <CardHeader className="rounded-t-2xl border-b border-gray-100 bg-gradient-to-r from-primary-50 to-white">
                        <CardTitle className="text-base sm:text-lg font-bold text-primary-700">This Month's Expenses ({new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })})</CardTitle>
                    </CardHeader>
                    <CardContent className="p-4 sm:p-6">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                            <div className="rounded-xl border border-primary-100 bg-gradient-to-br from-primary-50 to-white p-3 sm:p-4 transition-all hover:shadow-md">
                                ...
                                <p className="text-lg sm:text-xl font-bold text-primary-700">{formatCurrency(monthlyOffice)}</p>
                            </div>
                            <div className="rounded-xl border border-purple-100 bg-gradient-to-br from-purple-50 to-white p-3 sm:p-4 transition-all hover:shadow-md">
                                ...
                                <p className="text-lg sm:text-xl font-bold text-purple-700">{formatCurrency(monthlyTech)}</p>
                            </div>
                            <div className="rounded-xl border border-orange-100 bg-gradient-to-br from-orange-50 to-white p-3 sm:p-4 transition-all hover:shadow-md">
                                ...
                                <p className="text-lg sm:text-xl font-bold text-orange-700">{formatCurrency(monthlySnacks)}</p>
                            </div>
                            <div className="rounded-xl border border-teal-100 bg-gradient-to-br from-teal-50 to-white p-3 sm:p-4 transition-all hover:shadow-md">
                                ...
                                <p className="text-lg sm:text-xl font-bold text-teal-700">{formatCurrency(monthlyUtility)}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    <Card className="border-0 shadow-lg">
                        <CardHeader className="rounded-t-2xl border-b border-gray-100 bg-gradient-to-r from-primary-50 to-white">
                            <CardTitle className="text-base sm:text-lg font-bold text-primary-600">Income by Category ({year})</CardTitle>
                        </CardHeader>
                        <CardContent className="p-4 sm:p-6">
                    ...
                    <Card className="border-0 shadow-lg">
                        <CardHeader className="rounded-t-2xl border-b border-gray-100 bg-gradient-to-r from-red-50 to-white">
                            <CardTitle className="text-base sm:text-lg font-bold text-red-600">Expenses by Category ({year})</CardTitle>
                        </CardHeader>
                        <CardContent className="p-4 sm:p-6">
                                    ...
                                    <tbody>
                                        {recentTransactions.map((tx) => (
                                            <tr key={tx.id} className="border-b border-gray-50 transition-colors hover:bg-primary-50/30">
                                                <td className="px-4 sm:px-6 py-3 text-gray-600 whitespace-nowrap">{new Date(tx.transaction_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }).replace(/ /g, '-')}</td>
                                                ...
                                                <td className="hidden md:table-cell px-4 sm:px-6 py-3 text-gray-600">{tx.income_heading?.category?.name || tx.expense_heading?.category?.name || '-'}</td>
